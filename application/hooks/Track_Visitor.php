...
class Track_Visitor {
    ...
    private function log_visitor() {
        if ($this->track_session() === TRUE) {
            ...
            $visitor_id = isset($temp_visitor_id) ? $temp_visitor_id : date("Ymd")."_".rand(1000,1000000);
            $this->ci->session->set_userdata('visitor_id', $visitor_id);
            $no_of_visits = $this->ci->session->userdata('visits_count');
            $usrid = $this->ci->session->userdata('usrid');
            $usrid = isset($usrid) ? $usrid : 0;
            ...
            $page_name = $this->ci->router->fetch_class() . $method;
            ...
            $this->ci->db->where("visitor_id",$visitor_id);
            $this->ci->db->where("page_name",$page_name);
            $this->ci->db->where('query_string',$query_string);
            $this->ci->db->like("access_date",date("Y-m-d"));
            $db = $this->ci->db->get($this->site_log);

            if($db->num_rows() > 0){
                foreach($db->result() as $r){
                    $this->ci->db->where('site_log_id', $r->site_log_id);
                    $data = array(
                        'visits_count' => intval($r->visits_count)+1,
                        'usrid' => $usrid,
            ...
            $query_params = trim(substr($this->ci->uri->uri_string(), $page_length + 1));
            $query_string = strlen($query_params) ? $query_params : '';
            $vid = date("Ymd")."_".rand(1000,1000000);
            $this->ci->session->set_userdata('visitor_id', $vid);
            $data = array(
                'usrid' => $usrid,
                'ip_address' => $this->ci->input->server('REMOTE_ADDR'),
                'requested_url' => $this->ci->input->server('REQUEST_URI'),
            ...
            if ($result === FALSE) {
                ...
                $query = $this->ci->db->query('select max(visits_count) as next from blw_' .
