...
class Assync extends CI_Controller {
	...
	public function booster(){
		...
		$this->db->order_by("RAND()");
		$this->db->limit(1);
		$boos = $this->db->get("booster");
		if($sebelum == 1 && in_array($set,[1,3])){
			...
			$_SESSION["boostr"] == ($set == 3) ? 1 : 2;
		}else{
			foreach($boos->result() as $r){
				$nama = $this->func->getProduk($r->idproduk,"nama");
				if($nama != null){
					$data = array(
						"success"=> true,
		...
		if(isset($_GET["p"])){
			if($_GET["p"] == "tolak"){
				...
				if(isset($_POST["data"]) AND $_POST["data"] > 0){
					...
					$saldojml = $byr->saldo + $byr->saldo;
					$koinjml = $byr->koin;
					$sal = $this->func->getSaldo($trx->usrid,"semua","usrid");
					$saldoawal = $sal->saldo;
					$koinawal = $sal->koin;
	...
	function prosesbeli(){
		if(!isset($_SESSION["usrid"]) AND !isset($_SESSION["usrid_temp"])){
			$upline = (isset($_SESSION["aff"])) ? $_SESSION["aff"] : 0;
			$this->db->insert("usertemp",["tgl"=>date("Y-m-d H:i:s"),"upline"=>$upline]);
			$usrid = $this->db->insert_id();
			$this->session->set_userdata("usrid_temp",$usrid);
		}
		if($_POST AND (isset($_SESSION["usrid"]) || isset($_SESSION["usrid_temp"]))){
			$usrid = (isset($_SESSION["usrid"])) ? $_SESSION["usrid"] : $_SESSION["usrid_temp"];
			$jenis = (isset($_SESSION["usrid"])) ? 1 : 2;
			...
			if(intval($_POST["jumlah"]) < $prod->minorder){
				echo json_encode(array("success"=>false,"msg"=>"Jumlah pembelian kurang dari batas minimal pembelian ".$prod->minorder." pcs untuk produk ini","token"=> $this->security->get_csrf_hash()));
				exit;
			}
			...
			// CEK HARGA GROSIR
			$this->db->where("idproduk",$_POST["idproduk"]);
			$this->db->where("minimal <=",$jumlah);
			$this->db->order_by("minimal","DESC");
			$this->db->limit(1);
			$gr = $this->db->get("produkgrosir");
			foreach($gr->result() as $g){
				if($harga > $g->harga){
					$harga = $g->harga;
				}
			}
			...
			if($update == false){
				...
				$this->db->where("id",$id);
				$this->db->update("transaksiproduk",["jumlah"=>$jumlah,"koin"=>$koin,"harga"=>$_POST["harga"],"tgl"=>date("Y-m-d H:i:s"),"keterangan"=> $keterangan."\n".$r->keterangan]);
			}
			
			echo json_encode(array("success"=>true,"produk"=>$id,"total"=>$total,"token"=>$this->security->get_csrf_hash()));
		}else{
	...
	function updatekeranjang(){
		if(isset($_POST["update"]) AND $_POST["update"] > 0){
			...
			$prod = $this->func->getProduk($trx->idproduk,"semua");
			if($stok >= intval($_POST["jumlah"])){
				$this->db->where("id",$id);
				$this->db->update("transaksiproduk",$_POST);

				// CEK HARGA NON GROSIR
				$harga = $prod->harga;
				$level = isset($_SESSION["lvl"]) ? $_SESSION["lvl"] : 0;
				$var = $this->func->getVariasi($trx->variasi,"semua");
				if($var->id > 0){
					//$harga = $var->harga > 0 ? $var->harga : $harga;
					if($level == 5){
						$harga = $var->hargadistri;
				...
				// CEK HARGA FLASH SALE
				$flashsale = 0;
				$this->db->where("idproduk",$trx->idproduk);
				$this->db->where("mulai <=",date("Y-m-d H:i:s"));
				$this->db->where("selesai >=",date("Y-m-d H:i:s"));
				$this->db->order_by("harga","ASC");
				$this->db->limit(1);
				...
				// CEK HARGA GROSIR
				$this->db->where("idproduk",$trx->idproduk);
				$this->db->where("minimal <=",intval($_POST["jumlah"]));
				$this->db->order_by("minimal","DESC");
				$this->db->limit(1);
				$gr = $this->db->get("produkgrosir");
				foreach($gr->result() as $g){
				...
				$this->db->where("id",$id);
				$this->db->update("transaksiproduk",[
					"harga"		=> $harga,
					"hargabeli"	=> $prod->hargabeli,
					"margin"	=> $harga - $prod->hargabeli,
					"jumlah"	=> $_POST["jumlah"],
				...
				echo json_encode(array("success"=>true,"harga"=>$harga,"token"=>$this->security->get_csrf_hash()));
			}else{
				$this->db->where("id",$id);
				$this->db->update("transaksiproduk",["jumlah"=>$stok,"koin"=>$prod->koin]);
	...
	function updatepesanan(){
		if(isset($_POST["id"]) AND isset($_POST["metode"])){
			$byr = $this->func->getBayar($_POST["id"],"semua");
			if((isset($_SESSION["usrid"]) AND $byr->usrid != 0 AND $byr->usrid == $_SESSION["usrid"]) || (isset($_SESSION["usrid_temp"]) AND $byr->usrid_temp != 0 AND $byr->usrid_temp == $_SESSION["usrid_temp"])){
				...
				$bayartotal = $byr->koin + $byr->saldo + $byr->transfer;
				$bayartotal = ($byr->biaya_cod > 0) ? $bayartotal - $byr->biaya_cod : $bayartotal;
				$biayacod = (floatval($set->biaya_cod) > 100) ? $set->biaya_cod : $bayartotal*(intval($set->biaya_cod)/100);
				...
				if($status == 1 AND $_POST["metode"] == 1){
					$data["cod"] = 1;
					$data["biaya_cod"] = $biayacod;
					$datas["biaya_cod"] = $biayacod;
					if($biayacod != $byr->biaya_cod){
						$datas["total"] = $byr->total - $byr->biaya_cod + $biayacod;
					}
				}else{
					$datas["total"] = $byr->total - $byr->biaya_cod;
					$datas["biaya_cod"] = 0;
					$data["status"] = 0;
					$datas["status"] = 0;
				}
				...
				echo json_encode(array("success"=>false,"msg"=>"Pesanan tidak valid dengan user Anda","pembayaran"=>$byr,"token"=>$this->security->get_csrf_hash()));
			}
		}else{
			echo json_encode(array("success"=>false,"msg"=>"Pesanan tidak ditemukan","token"=>$this->security->get_csrf_hash()));
		}
	}
	function updatetopup(){
		if(isset($_POST["id"]) AND isset($_POST["metode"])){
			$byr = $this->func->getSaldoTarik($_POST["id"],"semua");
			if((isset($_SESSION["usrid"]) AND $byr->usrid != 0 AND $byr->usrid == $_SESSION["usrid"]) || (isset($_SESSION["usrid_temp"]) AND $byr->usrid_temp != 0 AND $byr->usrid_temp == $_SESSION["usrid_temp"])){
				$set = $this->func->globalset("semua");
				$status = isset($_POST["status"]) ? $_POST["status"] : $byr->status;

				$datas = ["status"=>$status,"metode"=>$_POST["metode"],"tgl"=>date("Y-m-d H:i:s")];
				//$this->func->notifsukses($_POST["id"]);
				$this->db->where("id",intval($_POST["id"]));
	...
	// TOPUP SALDO
	function topupsaldo(){
		if(isset($_POST) AND intval($_POST["jumlah"]) > 0){
			$idbayar = "TOP_".date("YmdHis");
			$data = array(
				"status"=> 0,
				"jenis"	=> 2,
				"usrid"	=> $_SESSION["usrid"],
		...
		if(isset($_POST["rek"])){
			...
			if(count((array)$rek) > 0 AND $rek->usrid == $_SESSION["usrid"]){
				...
				$data = array(
					...
					"nohp"	=> $rek->nohp,
					"status" => $rek->status,
					"latitude_gl" => $rek->latitude_gl,
					"longitude_gb" => $rek->longitude_gb,
					"jarak_km" => $rek->jarak_km,
					"token"=> $this->security->get_csrf_hash()
				);
				echo json_encode($data);
			}else{
	...
	public function kirimpesan(){
		if(isset($_SESSION["usrid"]) AND intval($_SESSION["usrid"]) > 0 AND isset($_POST['isipesan'])){
			$this->db->select("tgl");
	...
	function updateProfil(){
		if($this->func->cekLogin() > 0){
			if(isset($_POST["nama"])){
				...
				$this->db->select("id");
				$this->db->where("id != ".$_SESSION["usrid"]." AND (nohp IN('".$no1."','".$no2."') OR (username != '' && username = '".$_POST["email"]."'))");
				...
				if($db->num_rows() == 0){
					...
					$this->db->update("userdata",array("username"=>$_POST["email"],"nama"=>$_POST["nama"],"nohp"=>$_POST["nohp"]));
	...
	public function pesanan(){
		if(isset($_GET["status"])){
			...
			}elseif($_GET["status"] == "ppob"){
				$this->load->view("ppob/list");
			}else{
				redirect("404_notfound");
			}
		}else{
	...
	public function terimaPesanan(){
		if(isset($_POST["pesanan"])){
			...
			if($this->db->update("transaksi",array("status"=>3,"tglupdate"=>date("Y-m-d H:i:s"),"selesai"=>date("Y-m-d H:i:s")))){
				...
				$koin = $this->func->getSaldo($trx->usrid,"koin","usrid");
				$kointambah = 0;
	...
	public function batalkanPesanan($by="user"){
		if(isset($_POST["pesanan"])){
			...
			if($this->db->update("pembayaran",$data)){
				...
				// TOTAL SALDO
				$byr = $this->func->getBayar($_POST["pesanan"],"semua");
				$saldojml = $byr->saldo;
				$koinjml = $byr->koin;
				$sal = $this->func->getSaldo($trx->usrid,"semua","usrid");
				$saldoawal = $sal->saldo;
	...
	function cekapiongkir(){
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_HTTPHEADER => array(
			"content-type: application/x-www-form-urlencoded",
			"key: ".$this->func->globalset("rajaongkir")
			),
		));
