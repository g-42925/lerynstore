<?php
    ...
    $sal = isset($_SESSION["usrid"]) ? $this->func->getSaldo($_SESSION["usrid"],"semua","usrid") : 0;
...
<div class="row">
    <div class="col-md-8 m-b-30">
        <form id="cekout">
            ...
            <input type="hidden" id="koinval" value="<?=$koin;?>" />
            ...
            <input type="hidden" name="bayar_koin" id="bayar_koin" value="0" />

            <?php if($pre->digital == 1 AND $pre->usrid_temp > 0){ ?>
    ...
    <div class="col-md-4">
        ...
        <div class="section p-all-24">
            <?php if($koin > 0){ ?>
            <div class="fs-20 font-medium m-b-20">Gunakan Koin</div>
            <div class="metode m-b-20">
                <div class="m-b-12 p-all-12 m-lr-0 metodebayar" id="bayarkoin" data-bayar="koin">
                    <i class="fas fa-check-circle"></i>
                    <b class="fs-16 font-medium text-warning"><i class="fas fa-coins"></i> &nbsp;Gunakan Koin</b><br/>
                    <span class="fs-12">Koin saat ini Rp. <?=$this->func->formUang($koin)?></span>
                </div>
                <div class="bg-medium p-all-8 text-center fs-12" style="border-radius:8px;display:none" id="koinkurang">
                    Koin Anda saat ini kurang dari total tagihan pembayaran, maka sisanya silahkan lunasi dengan pilihan metode pembayaran dibawah
                    <div class="font-medium p-t-12 fs-14">Sisa Pembayaran:<br/><span class="text-danger font-bold" id="sisabayarkoin">Rp. 0</span></div>
                </div>
            </div>
            <?php } ?>

            <!--<div class="fs-20 font-medium m-b-20 metodekoin">Pilih Metode Pembayaran</div>-->
            <?php if(isset($_SESSION["usrid"]) AND intval($_SESSION["usrid"]) > 0){ ?>
            <div class="metode m-b-20 metodekoin">
            ...
            -->
            <div class="error text-danger" id="error-bayar" style="display:none;">
    ...
    $(function(){
        ...
        $(".methods").on("click",function(){
            ...
            if($(this).data("metode") == 1){
                $(".codon").show();
                var total = parseInt($("#subtotal").val()) + parseInt($("#ongkir").val()) + parseInt($("#biayacod").val()) - parseFloat($("#diskon").val()) - parseFloat($("#bayar_koin").val());
                ...
                if(valmet == 1){
                    var total = parseInt($("#subtotal").val()) + parseInt($("#ongkir").val()) - parseFloat($("#diskon").val()) - parseFloat($("#bayar_koin").val());// - parseInt($("#biayacod").val());
                    var minus = total - saldo;
                    //$("#total").val(total);
                    $("#totalbayar").html(formUang(total.toString()));
                    $(".codon").hide();
        ...
        $("#bayarsaldo").click(function(){
            if($(this).hasClass("active") == false){
                ...
                var total = parseInt($("#subtotal").val()) + parseFloat($("#ongkir").val()) - parseFloat($("#diskon").val()) - parseFloat($("#bayar_koin").val());
                var minus = total - saldo;
            ...
            if($(this).hasClass("active") == false){
                ...
                if(koin > 0){
                    ...
                    if(koin >= total){
                        ...
                        $("#koinkurang").show();
                        $("#bayar_koin").val(koin);
                        $("#sisabayarkoin").html("Rp. "+formUang(minus.toString()));
                        //hideCekout();
                        //$("#saldo").html("Rp "+formUang(saldo));
                        //$("#transfer").html("Rp "+formUang(selisih));
                    }
		...
		if($("#kodevoucher").val() != ""){
			$.post("<?=site_url("checkout/kupon")?>",{"kode":$("#kodevoucher").val(),"harga":$("#totalharga").val(),[$("#csrf_name").val()]: $("#csrf_token").val(),"ongkir":$("#ongkir").val()},function(msg){
				...
				if(data.success == true){ 
					var total = parseFloat($("#subtotal").val()) + parseFloat($("#ongkir").val()) - data.diskon - parseFloat($("#bayar_koin").val());
    ...
    function checkoutNow(){
        ...
        */
        ...
        if($("#cekout").valid() == true){
            ...
            swal.fire("Form belum lengkap","Formulir diatas mohon dilengkapi dulu","error");
            $(".pembayaran").show();
            $("#proses").hide();
        }
    }
