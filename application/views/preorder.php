			...
			<div class="p-b-50">
					...
					<div class="row produk-wrap">
						<?php
							...
							$where = "(nama LIKE '%$cari%' OR harga LIKE '%$cari%' OR hargareseller LIKE '%$cari%' OR hargaagen LIKE '%$cari%' OR deskripsi LIKE '%$cari%') AND preorder = 1 AND status = 1 AND stok > 0";
							$this->db->where($where);
							if(count($notin) > 0){
