<div class="table-responsive">
	<table class="table table-condensed table-hover">
		...
		<?php
			...
			$this->db->limit(20);
			$al = $this->db->get("transaksiproduk");
			foreach($al->result() as $l){
				if($l->idtransaksi > 0){
					$arrb[] = $this->func->getTransaksi($l->idtransaksi,"idbayar");
				}
			}
			/*
			$this->db->select("idbayar");
			$this->db->like("catatan",$cari);
			$this->db->limit(20);
			$al = $this->db->get("transaksi");
			foreach($al->result() as $l){
				if($l->idbayar > 0){
					$arrb[] = $l->idbayar;
				}
			...
			*/
			...
			$where = "(invoice LIKE '%$cari%' OR total LIKE '%$cari%' OR kodebayar LIKE '%$cari%' OR id IN(".$bin.") OR usrid IN(".$in.") OR usrid_temp IN(".$int.")) AND status = 0";
			$this->db->select("id");
			$this->db->where($where);
				...
				foreach($pro->result() as $r){
					...
					if($dbs->num_rows() > 0){
						foreach($dbs->result() as $res){
							$bukti = $res->bukti;
							$tgl .= "<br/><a href='javascript:void(0)' onclick='bukti(\"".base_url("cdn/konfirmasi/".$res->bukti)."\")'>&raquo; Lihat Bukti Transfer</a>";
						}
					}
...
<script type="text/javascript">	
	function konfirm(id){
		swal.fire({
			...
			if(val.value){
				loadingDulu();
				$.post("<?=site_url($this->func->admurl()."/api/updatepesanan")?>",{"id":id,"statusbayar":1,[$("#names").val()]:$("#tokens").val()},function(e){
					var data = eval("("+e+")");
	...
	function batalin(id){
		swal.fire({
			...
			loadingDulu();
			if(val.value){
				$.post("<?=site_url($this->func->admurl().'/api/batalkanpesanan')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(e){
					var data = eval("("+e+")");
					updateToken(data.token);
					if(data.success == true){
