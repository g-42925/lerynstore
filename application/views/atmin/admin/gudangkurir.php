...
<div class="p-tb-10">
	<form id="kurirform">
		<div class="row m-lr-0">
			<?php
				$kurir = explode("|",$this->admfunc->getGudang($id,"kurir"));
		...
		$(".kurir").each(function(){
			$(this).click(function(){
				...
				if($(this).data("aktif") == "no"){
					$.post("<?php echo site_url($this->func->admurl()."/gudang/aktifkankurir/".$id); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
						var data = eval("("+msg+")");
					...
					if(hitungJumlah() > 1){
						$.post("<?php echo site_url($this->func->admurl()."/gudang/nonaktifkankurir/".$id); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
							var data = eval("("+msg+")");
							updateToken(data.token);
							if(data.success == true){
