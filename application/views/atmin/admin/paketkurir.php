...
<script type="text/javascript">
	$(function(){
		...
		$("#kurirform").on("submit",function(e){
            ...
            $.post("<?=site_url($this->func->admurl()."/api/kurirsave")?>",datar,function(msg){
                var data = eval("("+msg+")");
		...
		$("#paketform").on("submit",function(e){
            ...
            var datar = $(this).serialize();
            datar = datar + "&" + $("#names").val() + "=" + $("#tokens").val();
            $.post("<?=site_url($this->func->admurl()."/api/paketsave")?>",datar,function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                if(data.success == true){
                    loadKurir(1);
		...
		$("#ongkirform").on("submit",function(e){
            ...
            var datar = $(this).serialize();
            datar = datar + "&" + $("#names").val() + "=" + $("#tokens").val();
            $.post("<?=site_url($this->func->admurl()."/api/ongkirsave")?>",datar,function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                if(data.success == true){
                    loadOngkir(1);
        ...
        $("#prov").change(function(){
            $.post("<?=site_url($this->func->admurl()."/api/getkab")?>",{"id":$(this).val(),[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                $("#kab").html(data.html);
            });
        ...
        $("#kab").change(function(){
            $.post("<?=site_url($this->func->admurl()."/api/getkec")?>",{"id":$(this).val(),[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                $("#kec").html(data.html);
            });
        ...
        $("#ongkirkurir").change(function(){
            $.post("<?=site_url($this->func->admurl()."/api/getpaketdrop")?>",{"id":$(this).val(),[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
    ...
    function selectKec(prov,kab,kec){
        $.post("<?=site_url($this->func->admurl()."/api/getkab/")?>"+kab,{"id":prov,[$("#names").val()]:$("#tokens").val()},function(msg){
            var data = eval("("+msg+")");
            updateToken(data.token);
            $("#kab").html(data.html);
            
			/*
            $.post("<?=site_url($this->func->admurl()."/api/getkec/")?>"+kec,{"id":kab,[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                $("#kec").html(data.html);
            });
    ...
    function selectPaket(kurir,paket){
        $.post("<?=site_url($this->func->admurl()."/api/getpaketdrop/")?>"+paket,{"id":kurir,[$("#names").val()]:$("#tokens").val()},function(msg){
            var data = eval("("+msg+")");
            updateToken(data.token);
            $("#ongkirpaket").html(data.html);
        });
	...
	function loadKurir(page){
		...
		$("#load").load("<?=site_url($this->func->admurl()."/api/kurir?page=")?>"+page);
	}
	function loadOngkir(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url($this->func->admurl()."/api/ongkir?page=")?>"+page);
	}
	...
	function editKurir(id){
		$.post("<?=site_url($this->func->admurl().'/api/getkurir')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$('#kurirform')[0].reset();
            $('#kuririd').val(data.id);
	...
	function editPaket(id){
		$.post("<?=site_url($this->func->admurl().'/api/getpaket')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
            $('#paketform')[0].reset();
            $('#paketid').val(data.id);
	...
	function editOngkir(id){
		$.post("<?=site_url($this->func->admurl().'/api/getongkir')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
            $('#ongkirform')[0].reset();
            ...
            $('#ongkirhargakm').val(data.hargakm);
            $('#ongkirkurir').val(data.kurir);
            $('#ongkirjenis').val(data.jenis);
            $('#prov').val(data.prov);
            //$('#kab').val(data.kab);
            //$('#kec').val(data.kec);
	...
	function hapusKurir(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/hapuskurir")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadKurir(1);
	...
	function hapusPaket(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/hapuspaket")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadKurir(1);
	...
	function hapusOngkir(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/hapusongkir")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadOngkir(1);
				...
				<form id="ongkirform">
					...
					<div class="form-group">
						...
						<select id="prov" class="form-control" required >
							...
							<option value="0">Nasional / Seluruh Indonesia</option>
							<?php
								$this->db->order_by("nama");
								$db = $this->db->get("prov");
								foreach($db->result() as $r){
					...
					-->
					<div class="form-group">
						<label>Ongkos Kirim & Jenis (Flat/Per KG)</label>
						<div class="row">
                            <div class="col-6"><input type="number" id="ongkirharga" name="harga" class="form-control" required /></div>
                            <div class="col-6">
								<select id="ongkirjenis" name="jenis" class="form-control" required >
									<option value="1">Flat</option>
									<option value="2">Per Kilogram (KG)</option>
								</select>
							</div>
                        </div>
					</div>
					<div class="form-group">
						<label>Hitungan Per Km</label>
						<div class="row">
                            <div class="col-6"><input type="number" id="ongkirhargakm" name="hargakm" class="form-control" required /></div>
                        </div>
					</div>
					<div class="form-group">
