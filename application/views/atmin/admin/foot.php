...
<script src="<?=base_url()?>assets/atmin/js/plugin/jquery-ui-1.12.1.custom/jquery-ui.min.js"></script>
<script src="<?=base_url()?>assets/atmin/js/core/popper.min.js"></script>
<script src="<?=base_url()?>assets/atmin/js/core/bootstrap.min.js"></script>
...
<script src="<?=base_url()?>assets/atmin/js/plugin/chartist/chartist.min.js"></script>
<script src="<?=base_url()?>assets/atmin/js/plugin/chartist/plugin/chartist-plugin-tooltip.min.js"></script>
<script src="<?=base_url()?>assets/atmin/js/plugin/bootstrap-notify/bootstrap-notify.min.js"></script>
<script src="<?=base_url()?>assets/atmin/js/plugin/bootstrap-toggle/bootstrap-toggle.min.js"></script>
<script src="<?=base_url()?>assets/atmin/js/plugin/jquery-mapael/jquery.mapael.min.js"></script>
<script src="<?=base_url()?>assets/atmin/js/plugin/chart-circle/circles.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/3.1.0/socket.io.js" integrity="sha512-+l9L4lMTFNy3dEglQpprf7jQBhQsQ3/WvOnjaN/+/L4i0jOstgScV0q2TjfvRF4V+ZePMDuZYIQtg5T4MKr+MQ==" crossorigin="anonymous"></script>
<script src="<?=base_url("assets/atmin/js/sweetalert2.min.js")?>" type="text/javascript"></script>
<script src="<?=base_url()?>assets/atmin/js/ready.min.js"></script>
<script type="text/javascript">
	...
	function playNotif(){
		$.post("<?=site_url($this->func->admurl()."/api/cekupdatenotif")?>",{[$("#names").val()]:$("#tokens").val()},function(e){
			var data = eval("("+e+")");
			//updateToken(data.token);
			if(data.jmlpesanan > 0 || data.jmlpesan > 0){
				var audio = new Audio('<?=base_url("assets/atmin/mp3/notifikasi.mp3")?>');
	...
	function logout(){
		swal.fire({
			...
			if(vals.value == true){
				window.location.href = "<?=site_url($this->func->admurl()."/manage/logout")?>";
			}
		});
	}
	...
	function resetToken(){
		$("#tokentempo").load("<?=site_url($this->func->admurl()."/api/resetoken")?>",function(){
