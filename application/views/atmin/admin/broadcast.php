<div class="m-b-60">
	<div class="card">
		...
		<div class="card-body">
			<?php if($this->admfunc->demo() == true){ ?>
			<div class="alert alert-warning">
				<i data-notify="icon" class="fas fa-exclamation-circle"></i>
				<span data-notify="message">Untuk mode demo, fitur broadcast ke email dan whatsapp kami nonaktifkan agar tidak mengganggu pengguna aplikasi demo</span>
			</div>
			<?php } ?>
			<div id="load">
...
<script type="text/javascript">
	$(function(){
		...
		$("#forms").on("submit",function(e){
			...
			$.ajax( {
                url        : '<?=site_url($this->func->admurl()."/api/tambahbroadcast")?>',
                type       : 'POST',
                contentType: false,
                cache      : false,
                processData: false,
	...
	function loadB(page){
		...
		$.post("<?=site_url($this->func->admurl()."/api/broadcast?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/hapusbroadcast")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
	...
	function resend(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/updatebroadcast")?>",{"id":id,"status":0,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
				...
				<form id="forms">
                    ...
                    <div class="form-group m-b-12">
                        ...
                        <select class="form-control" name="tujuan" id="tujuan" required>
                            ...
                            <option value="0">Semua Pengguna [ <?=$this->admfunc->getJmlUser()?> user ]</option>
                            <option value="1">Sudah Pernah Beli [ <?=$this->admfunc->getJmlUser(1)?> user ]</option>
                            <option value="2">Belum Pernah Beli [ <?=$this->admfunc->getJmlUser(2)?> user ]</option>
                            <option value="3">Ada Produk di Keranjang Belanja [ <?=$this->admfunc->getJmlUser(3)?> user ]</option>
                        </select>
                    </div>
