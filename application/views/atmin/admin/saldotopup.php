...
<script type="text/javascript">
	$(function(){
		...
		$("#sbforms").on("submit",function(e){
			...
			$.post("<?=site_url($this->func->admurl()."/api/tambahtopup")?>",datar,function(data){
				$("#sbforms").show();
				$(".progress").hide();
				var res = eval("("+data+")");
	...
	function loadTopup(page){
		...
		$.post("<?=site_url($this->func->admurl()."/api/topup?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function verifikasi(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/updatetopup")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
	...
	function batal(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/bataltopup")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
                        ...
                        <select class="select2" name="usrid" id="usrid" required>
                            ...
                            <?php
                                ...
                                foreach($dt->result() as $r){
                                    ...
                                    $profil = $this->admfunc->getProfil($r->id,"semua","usrid");
                                    $nohp = $profil->nohp != "" ? $profil->nohp : $r->username;
                                    echo "<option value='".$r->id."'>".$this->security->xss_clean($profil->nama." ".$nohp." ".$level)."</option>";
                                }
                            ?>
                        </select>
                    </div>
