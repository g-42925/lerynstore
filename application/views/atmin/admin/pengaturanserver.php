	...
	<div class="row">
		...
		<div class="col-md-6 m-b-20">
			...
			<div class="form-group wagw" <?php if($set->api_wasap != "wagw"){ echo 'style="display:none"'; } ?>>
				...
				<?php }else{ ?>
				<input type="text" id="wagw_nomer" name="wagw_nomer" class="form-control" value="<?=$set->wagw_nomer?>" />
				<?php } ?>
			</div>
			<div class="form-group wagw" <?php if($set->api_wasap != "wagw"){ echo 'style="display:none"'; } ?>>
				...
				<?php }else{ ?>
				<input type="text" id="wagw_domain" name="wagw_domain" class="form-control" value="<?=$set->wagw_domain?>" />
				<?php } ?>
			</div>
			<div class="form-group wagw" id="cekwa" <?php if($set->api_wasap != "wagw"){ echo 'style="display:none"'; } ?>>
			...
			<div class="form-group text-danger" id="cekwanotif" style="display:none">
				<i class="fas fa-info-circle"></i> Simpan terlebih dahulu sebelum Anda bisa melakukan Scan Kode QR dan Cek Koneksi
			</div>
			...
			<div class="form-group titel m-t-30" style="font-weight: bold;">
                GOOGLE AUTH & PUSH NOTIFICATION MOBILE	
			</div>
			<div class="form-group">
				<label>Server Key Token <b>Firebase Cloud Messaging</b></label>
				<?php if($this->func->demo() == true){ ?>
				<input type="text" name="fcm_serverkey" class="form-control" value="abcdefghijklmnopqrstuvwxyz1234567890" <?=$read?> />
				<?php }else{ ?>
				<input type="text" name="fcm_serverkey" class="form-control" value="<?=$set->fcm_serverkey?>" />
				<?php } ?>
			</div>
			<div class="form-group">
				<label><b>Google OAuth</b> Client ID</label>
				<?php if($this->func->demo() == true){ ?>
				<input type="text" name="google_client_id" class="form-control" value="abcdefghijklmnopqrstuvwxyz1234567890" <?=$read?> />
				<?php }else{ ?>
				<input type="text" name="google_client_id" class="form-control" value="<?=$set->google_client_id?>" />
				<?php } ?>
			</div>
			<div class="form-group">
				<label><b>Google OAuth</b> Client Secret</label>
				<?php if($this->func->demo() == true){ ?>
				<input type="text" name="google_client_secret" class="form-control" value="abcdefghijklmnopqrstuvwxyz1234567890" <?=$read?> />
				<?php }else{ ?>
				<input type="text" name="google_client_secret" class="form-control" value="<?=$set->google_client_secret?>" />
				<?php } ?>
			</div>
		</div>
...
<script type="text/javascript">
        ...
        <?php
			if($this->func->demo() == true){
				...
				echo '
					...
					$.post("'.site_url($this->func->admurl().'/api/savesetting').'",{"api_wasap":val,[$("#names").val()]:$("#tokens").val()},function(ev){
						var data = eval("("+ev+")");
        ...
        <?php
			if($this->func->demo() == true){
				...
				echo '
					...
					$.post("'.site_url($this->func->admurl().'/api/savesetting').'",{"login_otp":val,[$("#names").val()]:$("#tokens").val()},function(ev){
						var data = eval("("+ev+")");
						updateToken(data.token);
						if(val == 1){
	...
	$(function(){
        ...
        $("#wagw_nomer").keyup(function(){
            if($(this).val() != "<?=$set->wagw_nomer?>"){
                $("#cekwa").hide();
                $("#cekwanotif").show();
            }else{
                $("#cekwa").show();
        ...
        $("#wagw_domain").keyup(function(){
            if($(this).val() != "<?=$set->wagw_domain?>"){
                $("#cekwa").hide();
                $("#cekwanotif").show();
            }else{
                $("#cekwa").show();
	...
	function cekWA(){
        ...
        $('.areascanqr').html(`
			<div class="card-body">
				<div id="info-detik-${nomor}" class="m-b-20"></div>
				...
				<div class="text-center p-t-20"><button class="btn btn-danger scanbutton" onclick="logoutqr(${nomor})"><i class='fas fa-power-off'></i> &nbsp;Logout</button></div>
			</div>
		`);
    ...
    function logoutqr(nomor) {
		swal.fire({
			text: "koneksi ke Whatsapp di Handphone akan di hapus, dan setelah dihapus silahkan logout juga di Handphone Anda",
			title: "Yakin memutuskan koneksi Whatsapp ini?",
			type: "warning",
    ...
    socket.on('qr', function(src) {
        console.log(src)
        $(`#cardimg-${src.id}`).html(`<img src="` + src.src + `" class="card-img-top" alt="cardimg" id="qrcode" style="height:250px; width:250px;">`);
        $(`.info-detik-${src.id}`).html(`<p class='text-center'>Waktu scan anda adalah <span class="text-primary fs-20">13</span> detik</p>`);
        var count = 0;
        var interval = setInterval(function() {
            count++
            counts = 13 - count;
            $(`.info-${src.id}`).html(`<p class='text-center'>sisa waktu scan <span class="text-danger fs-24">${counts}</span> detik lagi</p>`);
            if (count == 13) {
                $(`.info-${src.id}`).html("");
                $(`#cardimg-${src.id}`).html(`<h5 class="text-center text-warning mt-4 m-b-12">Silahkan tutup dulu jendela ini lalu klik tombol scan lagi untuk scan ulang<h5>`);
    ...
    socket.on('authenticated', function(src) {
        ...
        $(`#cardimg-${src.id}`).html(`
			<div class='text-center'>
				...
				<div class='m-b-12'>Nomor WA : ${src.data.id}</div>
			</div>
            `);
