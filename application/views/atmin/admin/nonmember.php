...
<script type="text/javascript">
	...
	function loadB(page){
		...
		$.post("<?=site_url($this->func->admurl()."/member/getnonmember?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/member/hapusnonmember")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
