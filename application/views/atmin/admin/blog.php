	...
	<div class="card">
		<div class="card-header align-items-center">
			<div class="card-title">
				<a class="float-right btn btn-primary" href="<?=site_url($this->func->admurl()."/manage/editblog")?>"><i class="fas fa-plus-circle"></i> Postingan Baru</a>
				<h4 class="page-title" style="margin-bottom:0;">Postingan Blog</h4>
			</div>
		</div>
...
<script type="text/javascript">
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url($this->func->admurl()."/api/updateblog")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function loadBlog(page){
		...
		$.post("<?=site_url($this->func->admurl()."/manage/blog?load=hal&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function hapus(pro){
		swal.fire({
			...
			if(result.value){
				$.post("<?php echo site_url($this->func->admurl().'/api/hapusblog'); ?>",{"id":pro,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
