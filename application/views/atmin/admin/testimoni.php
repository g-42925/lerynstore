	...
	<div class="card">
		<div class="card-header align-items-center">
			<div class="card-title">
				...
				<h4 class="page-title" style="margin-bottom:0;">Testimoni Pembeli</h4>
			</div>
		</div>
...
<script type="text/javascript">
		...
		$("#sbforms").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.ajax({
						url        : '<?php echo site_url($this->func->admurl()."/api/tambahtestimoni"); ?>',
						type       : 'POST',
						contentType: false,
						cache      : false,
						processData: false,
	...
	function loadTesti(page){
		...
		$.post("<?=site_url($this->func->admurl()."/api/testimoni?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function edit(id){
		$.post("<?=site_url($this->func->admurl().'/api/testimoni')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#id").val(id);
			$("#nama").val(data.nama);
			...
			$("#blah").attr("src","<?=base_url()?>/cdn/uploads/"+data.foto);
			$("#blah").show();
			$(".delete").show();
			$(".text").hide()
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url($this->func->admurl()."/api/hapustestimoni")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
