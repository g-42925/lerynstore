...
<a href="<?=site_url($this->func->admurl()."/manage/produkform")?>" class="btn btn-success float-right m-r-10"><i class="fas fa-plus-circle"></i> Produk Baru</a>
...
<script type="text/javascript">
	$(function(){
		...
		$("#impor").on("submit",function(e){
			...
			$.ajax( {
                url        : '<?php echo site_url($this->func->admurl()."/api/import"); ?>',
                type       : 'POST',
                contentType: false,
                cache      : false,
                processData: false,
	...
	function refreshTabel(page){
		...
		$.post("<?=site_url($this->func->admurl()."/manage/produk?load=true")?>&page="+page+"&perpage="+perpage,{"cari":$("#cari").val(),"status":$("#status").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function sundul(id){
		$.post("<?=site_url($this->func->admurl()."/produk/sundul")?>",{"id":id},function(msg){
			var data = eval("("+msg+")");
			if(data.success == true){
				swal.fire("Berhasil","produk telah diupdate jadi paling teratas","success").then((val)=>{
					window.location.reload();
				});
			}else{
				swal.fire("Gagal!","gagal mengupdate data, cobalah beberapa saat lagi","error");
			}
		});
	}
	function hapus(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url($this->func->admurl()."/api/hapusproduk")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
							window.location.href="<?=site_url($this->func->admurl()."/produk")?>";
						});
					}else{
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="m-b-30">
					...
					<a href="<?=base_url("cdn/import/Template_Import.xlsx")?>" class="btn btn-link"><i class="fas fa-file-download"></i> &nbsp;download template impor</a>
				</div>
				<form id="impor">
