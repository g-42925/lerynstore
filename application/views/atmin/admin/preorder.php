...
<script type="text/javascript">
	...
	function loadPreorder(page){
		...
		$.post("<?=site_url($this->func->admurl()."/manage/preorder?load=preorder&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function loadProduk(page){
		...
		$.post("<?=site_url($this->func->admurl()."/manage/preorder?load=produk&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	}
	function loadPerProduk(page){
		$("#loads").html('<i class="fas fa-spin fa-compact-disc"></i> Loading data...');
		$.post("<?=site_url($this->func->admurl()."/manage/preorder?load=preorder&page=")?>"+page+"&idproduk="+$("#idproduk").val(),{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function detailPesanan(id){
		$("#idproduk").val(id);
		$.post("<?=site_url($this->func->admurl()."/manage/preorder?load=preorder&page=1&idproduk=")?>"+id,{[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#loads").html(data.result);
			$("#modal").modal();
