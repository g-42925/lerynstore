...
<?php
	...
	if($id != 0 || (isset($_GET["copy"]) AND intval($_GET["copy"]) > 0)){
		if(isset($_GET["copy"])){
			$id = intval($_GET["copy"]);
			$url = site_url($this->func->admurl()."/api/tambahproduk");
			$varjum = 0;
			$this->session->unset_userdata("fotoCopy");
		}else{
			$url = site_url($this->func->admurl()."/api/updateproduk");
			$varjum = $this->admfunc->getVariasiJumlah($id);
		}
		$data = $this->admfunc->getProduk($id,"semua");
		if($data->id == 0){
			redirect($this->func->admurl()."/manage/produk");
			exit;
		}else{
...
<form id="produk" action="" method="POST">
	...
	<div class="card">
		...
		<div class="card-body">
			...
			<div class="row m-lr-0 m-b-24">
				<div class="col-md-4">
					<div class="m-b-4"><b>Brand</b> &nbsp;<span class="badge badge-form">wajib</span></div>
					<div class="fs-12">Pilih kategori brand yang sesuai dengan produk.</div>
				</div>
				<div class="col-md-8">
					<select class="select2" name="brandid" required>
						<option value="">- Pilih Brand -</option>
						<?php
							$kat = $this->db->get("brand");
							foreach($kat->result() as $r){
								$selec = ($id != 0 AND $data->brandid == $r->id) ? "selected" : "";
								echo "<option value='".$r->id."' $selec>".$r->nama."</option>";
							}
						?>
					</select>
	...
	<div class="card">
		...
		<div class="card-body">
			...
			<div class="row m-lr-0 m-b-24">
				<div class="col-md-4">
					<div class="m-b-4"><b>Notif Terjual</b> &nbsp;<span class="badge badge-form">wajib</span></div>
					<div class="fs-12">Notif produk terjual yg ditampilkan di item produk untuk mempengaruhi pembeli.</div>
				</div>
				<div class="col-md-3 col-6">
					<input type="number" class="form-control" name="customterjual" value="<?php echo ($id != 0) ? ($data->customterjual ?? 0) : 1; ?>" required />
				</div>
			</div>
			<div class="row m-lr-0 m-b-24">
				<div class="col-md-4">
			...
			<div class="row m-lr-0 m-b-24">
				...
				<div class="col-md-8">
					<div class="form-group">
						<textarea id="summernote" name="deskripsi"><?php echo ($id != 0) ? $data->deskripsi : ""; ?></textarea>
					</div>
				</div>
			</div>
			<div class="row m-lr-0 m-b-24">
		...
		<div class="col-md-6">
			<div class="card">
				<div class="card-header row m-lr-0">
					<div class="card-title p-lr-0">Custom Label</div>
				</div>
				<div class="card-body">
					<table class="table">
						<thead>
						...
						<tbody id="customlabel">
							...
							<tr id="label<?=$nos?>">
								<th>
									<input type="text" class="form-control" name="labeltext[]" value="<?=$lab->text?>" />
								</th>
								<th>
									<input type="color" class="form-control py-0 px-0" name="labelwarna[]" value="<?=$lab->warna?>" />
								</th>
								<th>
									<input type="color" class="form-control py-0 px-0" name="labelbackground[]" value="<?=$lab->background?>" />
								</th>
								<th>
									<button type="button" class="btn btn-danger btn-sm" onclick="hapusLabel('label<?=$nos?>')"><i class="fas fa-times"></i></button>
								</th>
							</tr>
							<?php
											$nos++;
					...
					<button type="button" class="btn btn-primary btn-block" onclick="tambahLabel()"><i class="fas fa-plus"></i> &nbsp;Tambah Label</button>
				</div>
			</div>
		</div>
		<div class="col-md-6">
				...
				<div class="card-body">
					<div id="grosirform">
						...
						<div class="m-b-12 grosir" <?=$gdis?>>
							<div class="row">
								<div class="col-4">Minimal Order</div>
...
<div class="m-b-100">
	...
	<div id="tambahlabel" style="display:none;">
		<table>
			<tbody>
				<tr>
					<th>
					...
					<th>
						<input type="color" class="form-control py-0 px-0" name="labelwarna[]" />
					</th>
					<th>
						<input type="color" class="form-control py-0 px-0" name="labelbackground[]" />
					</th>
					<th>
						<button type="button" class="btn btn-danger btn-sm hapuslabel" onclick="hapusLabel()"><i class="fas fa-times"></i></button>
					</th>
				</tr>
			</tbody>
		</table>
...
<script type="text/javascript">
	...
	function tambahLabel(){
		var id = makeid(8);
		$("#tambahlabel tr").attr('id',id);
		$("#tambahlabel .hapuslabel").attr('onclick',"hapusLabel('"+id+"')");
		$("#customlabel").append($("#tambahlabel table tbody").html());
		setTimeout(() => {
			$("#tambahlabel tr").attr('id',"customlabeltambahid");
		}, 500);
	}
	function hapusLabel(id){
		$("#"+id).remove();
    }
	...
	function hapusGrosir(elem,id){
		...
		if(id > 0){
			swal.fire({
				...
				if(val.value){
					$.post("<?=site_url($this->func->admurl().'/api/hapusgrosir')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(e){
						var data = eval("("+e+")");
	...
	$(function(){
		...
		$("#variasi").on('click','.hapusvariasion',function(){
			...
			swal.fire({
				...
				if(val.value){
					$.post("<?=site_url($this->func->admurl().'/manage/hapusvariasi')?>",{"theid":varid,[$("#names").val()]:$("#tokens").val()},function(e){
						var data = eval("("+e+")");
						updateToken(data.token);
						if(data.success == true){
							therem.remove();
		...
		$("#produk").on("submit",function(e){
			...
			formData.append("variasi",$("#jdlvariasi").val());
			formData.append("subvariasi",$("#jdlsubvariasi").val());
			formData.delete("deskripsi");
			formData.append("deskripsi",$("#summernote").summernote('code'));
			// "&" + $("#names").val() + "=" + $("#tokens").val() + 

			//$.post("",formData,function(msg){
			$.ajax({
				type: "POST",
				url: "<?=$url?>",
				data: formData,
					...
					if(data.success == true){
						...
						*/
									swal.fire("Selesai","Data produk telah disimpan","success").then((val)=>{
										window.location.href="<?=site_url($this->func->admurl()."/manage/produk")?>";
									});
						/*
								}else{
		...
		$("#fotoUpload").change(function(){
			...
			$.ajax( {
                url        : '<?php echo site_url($this->func->admurl()."/api/uploadFotoProduk"); ?>',
                type       : 'POST',
                contentType: false,
                cache      : false,
                processData: false,
	...
	function loadResult(){
		...
		$.post('<?php echo site_url($this->func->admurl()."/api/uploadFotoResult/".$id); ?>',{"response":212,[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			if(data.success == true){
	...
	function loadVariasi(){
		...
		$.post('<?php echo site_url($this->func->admurl()."/api/variasiform/".$id); ?>',{"response":212,[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			if(data.success == true){
	...
	function hapusFoto(id){
		if(id != "all"){
			swal.fire({
				...
				if(val.value){
					$.post('<?php echo site_url($this->func->admurl()."/api/hapusFotoProduk/"); ?>'+id,{"response":212,[$("#names").val()]:$("#tokens").val()},function(msg){
						var data = eval("("+msg+")");
						updateToken(data.token);
						if(data.success == true){
							loadResult();
	...
	function jadikanUtama(id){
		$.post('<?php echo site_url($this->func->admurl()."/api/jadikanFotoUtama/"); ?>'+id,{"idproduk":<?php echo $id; ?>,[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			if(data.success == true){
				loadResult();
