<div class="table-responsive">
	...
	<?php
		...
		$where = "(username LIKE '%$cari%' OR nama LIKE '%$cari%' OR nohp LIKE '%$cari%' OR tgl LIKE '%$cari%')";
		if($_GET["load"] == "distri"){
			$where .= " AND level = 5";
			$fungsi = "loadDistri";
			$head = "Distributor";
	...
	<?php
		...
		if($rows > 0){
			...
			foreach($db->result() as $r){
				$user = $this->admfunc->getProfil($r->id,"semua","usrid");
				...
				$saldo = $this->admfunc->getSaldo($r->id,"semua");
	?>
			<tr>
				<td><?=$no?></td>
				<td><?=$this->security->xss_clean($user->nama)?></td>
				<td><?=$user->nohp?></td>
				<td>Rp. <?=$this->admfunc->formUang($saldo->saldo)?></td>
				<td><?=$this->admfunc->formUang($saldo->koin)?></td>
				<td>Rp. <?=$this->admfunc->formUang($total)?></td>
				<td>
	...
	<?=$this->admfunc->createPagination($rows,$page,$perpage,$fungsi);?>
</div>