...
<div class="table-responsive">
	...
	<?php
		...
		if($rows > 0){
			...
			foreach($db->result() as $rs){
				...
				if(isset($_GET["idproduk"])){
					$this->db->where("idproduk",$_GET["idproduk"]);
				}
				$dbs = $this->db->get("transaksiproduk");
				$alamat = $this->admfunc->getAlamat($rs->alamat,"semua");
				$profil = $this->admfunc->getProfil($rs->usrid,"semua","usrid");
				...
				foreach($dbs->result() as $r){
					$konfirmasi = $this->admfunc->getKonfirmasi($rs->idbayar,"semua","idbayar");
	...
	?>
			<tr>
				...
				<td>
					tgl pesanan:<br/>
					<span class='text-primary'><?=$this->admfunc->ubahTgl("d/m/Y H:i",$rs->tglupdate)?></span>
					<?php if($rs->status < 2){ ?>
						<br/>perkiraan pesanan siap:<br/>
						<span class='text-danger'><?=$this->admfunc->ubahTgl("d/m/Y",$tgljadi)?></span>
					<?php }else{ ?>
						<br/>tgl pengiriman:<br/>
						<span class='text-danger'><?=$this->admfunc->ubahTgl("d/m/Y",$rs->kirim)?></span>
					<?php } ?>
				</td>
				<td><?=$pembeli?></td>
				<td><?=ucwords($produk->nama)?></td>
				<td><?=$this->admfunc->formUang($r->jumlah)?></td>
				<td>Rp. <?=$this->admfunc->formUang($r->jumlah*$r->harga)?></td>
			</tr>
	<?php	
					$no++;
				}
	...
	<?=$this->admfunc->createPagination($rows,$page,$perpage,$p);?>
</div>
<script type="text/javascript">
	function konfirm(id){
		swal.fire({
			...
			if(val.value){
				$.post("<?=site_url($this->func->admurl()."/api/updatepreorder")?>",{"id":id,"status":1,[$("#names").val()]:$("#tokens").val()},function(e){
					var data = eval("("+e+")");
...
<div class="table-responsive">
	...
	?>
			<tr>
				...
				<td><?=$this->admfunc->formUang($kuota)." pcs<br/><span class='text-primary'>".$this->admfunc->formUang($kuotatotal)."</span>"?></td>
				<td><?=$this->admfunc->formUang($kuotab)." pcs<br/><span class='text-danger'>".$this->admfunc->formUang($kuotabtotal)."</span>"?></td>
				<td><?=$this->admfunc->formUang($kuotas)." pcs<br/><span class='text-success'>".$this->admfunc->formUang($kuotastotal)."</span>"?></td>
				<td>
