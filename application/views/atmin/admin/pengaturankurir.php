			...
			?>
					<div class="col-md-3 col-6 kurir p-lr-5" data-push="<?php echo $kur->id; ?>" data-aktif="<?php echo $aktiv; ?>">
						<div class="imgk p-all-20 pointer<?php echo $op; ?>" style="<?php echo $bor; ?>">
							<?php if(file_exists("assets/images/kurir/".$kur->rajaongkir.".png")){ ?>
								<img class="w-full" src="<?=base_url("assets/images/kurir/".$kur->rajaongkir.".png"); ?>" />
							<?php }else{ ?>
		...
		$(".kurir").each(function(){
			$(this).click(function(){
				...
				if($(this).data("aktif") == "no"){
					$.post("<?php echo site_url($this->func->admurl()."/api/aktifkankurir"); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
						var data = eval("("+msg+")");
						updateToken(data.token);
						if(data.success == true){
							...
							swal.fire("Gagal!","Terjadi kendala saat mengaktifkan kurir ini, coba ulangi beberapa saat lagi<br/>"+data.msg,"error");
						}
					});
				}else{
					if(hitungJumlah() > 1){
						$.post("<?php echo site_url($this->func->admurl()."/api/nonaktifkankurir"); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
							var data = eval("("+msg+")");
							updateToken(data.token);
							if(data.success == true){
