<div class="table-responsive">
	<table class="table table-condensed table-hover">
		...
		<?php
			...
			$where = "cek = '0' AND status = '1' AND (invoice LIKE '%$cari%' OR bayar LIKE '%$cari%' OR usrid IN(".$in.") OR idproduk IN(".$int."))"; 
			$this->db->select("id");
			$this->db->where($where);
		...
		?>
			<tr>
				<td><b>#<?=$r->invoice?></b><br/><i class="fas fa-clock text-warning"></i> &nbsp;<?=$this->admfunc->ubahTgl("d/m/Y H:i",$r->tgl);?></td>
				...
				<td><?=$detail?></td>
				<td>
					<?php if($produk->tipe == 2){ ?>
					<button type="button" class="btn btn-primary" onclick="cekPesanan(<?=$r->id?>)">Cek Status</button>
					<?php } ?>
				</td>
			</tr>
		<?php	
				...
				echo "<tr><td colspan=5 class='text-center text-danger'>Belum ada pesanan</td></tr>";
			}
		?>
	</table>

	<?=$this->admfunc->createPagination($rows,$page,$perpage,"loadProses");?>
</div>