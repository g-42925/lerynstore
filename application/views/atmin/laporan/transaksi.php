		...
		<div class="card-header">
			<div class="row" style="align-items:center;">
				...
				<div class="col-md-3 p-tb-6">
					<div class="m-b-8">Gudang</div>
					<select id="gudang" onChange="loadRiwayat()" class="form-control">
						<option value="semua">Semua</option>
						<option value="0">PUSAT</option>
						<?php
							$db = $this->db->get("gudang");
							foreach($db->result() as $r){
								$kota = $this->admfunc->getKab($r->idkab,"semua");
								$kota = $kota->tipe." ".$kota->nama;
								echo '<option value="'.$r->id.'">'.$r->nama.' - '.$kota.'</option>';
							}
						?>
					</select>
				</div>
...
<script type="text/javascript">
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url($this->func->admurl()."/api/update")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function loadRiwayat(){
		...
		$.post("<?=site_url($this->func->admurl()."/laporan/transaksi?load=hal")?>",{"gudang":$("#gudang").val(),"status":$("#status").val(),"jenis":$("#jenis").val(),"tglmulai":$("#tglmulai").val(),"tglselesai":$("#tglselesai").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
