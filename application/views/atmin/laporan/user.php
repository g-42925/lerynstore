                        ...
                        <select id="usrid" class="select2">
							<?php
								...
								foreach($db->result() as $r){
									echo "<option value='".$r->id."'>".strtoupper(strtolower($this->admfunc->clean($r->nama)))." - ".$r->username." - ".$r->nohp."</option>";
								}
							?>
                        </select>
                    </div>
...
<script type="text/javascript">
	...
	function loadUser(level){
		$.post("<?=site_url($this->func->admurl()."/api/getusrid")?>?level="+level,{[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#usrid").html(data.load);
			//$("#usrid").trigger("change");
		});
	}
	function loadRiwayat(){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url($this->func->admurl()."/laporan/user?load=hal")?>",{"usrid":$("#usrid").val(),"tglmulai":$("#tglmulai").val(),"tglselesai":$("#tglselesai").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
