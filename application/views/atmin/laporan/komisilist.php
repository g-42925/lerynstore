...
<div class="table-responsive">
	...
	<?php
		...
		if($db->num_rows() > 0){
			...
			foreach($db->result() as $r){
				$trx = $this->admfunc->getTransaksi($r->idtransaksi,"semua");
				$bayar = $this->admfunc->getBayar($trx->idbayar,"semua");
				...
				if($r->status == 0){
					...
					$status = "Sudah Cair";
                    $status .= "<br/><span class='text-success'>".$this->admfunc->ubahTgl("d/m/Y H:i",$r->cair)."</span>";
				}elseif($r->status == 3){
					$status = "Dibatalkan";
				}else{
					$status = "-";
	...
	?>
			<tr>
				...
				<td class='text-right'><?=$this->admfunc->formUang($r->jumlah)?></td>
				<td><?=$status?></td>
			</tr>
	<?php	
				$no++;
			...
			if($total > 0){
				...
				<tr>
					...
					<th class='text-right'>Rp. ".$this->admfunc->formUang($totalkomisi)."</th>
