...
class Tripay_payment extends CI_Model{
        ...
        if(empty($err)){
            ...
            if($res["success"] == true AND count($res["data"]) > 0){
                for($i=0; $i<count($res["data"]); $i++){
                    $logo = (isset($logos[$res["data"][$i]["code"]])) ? base_url("assets/images/payment/".$logos[$res["data"][$i]["code"]]) : $res["data"][$i]["icon_url"]; //base_url("cdn/uploads/no-image.png");
    ...
    function createPayment($trx,$metode,$jumlah,$pembeli,$produk=null){
        $set = $this->set("semua");
        $globalset = $this->globalset("semua");
        $trx = $this->func->getBayar($trx,"semua");
        $produk = is_array($produk) ? $produk : [['sku'=>'BO1','name'=>'Pembelian di '.$globalset->nama,'price'=> $jumlah,'quantity'=>1]];
        $pembeli["nama"] = (isset($pembeli["nama"]) AND $pembeli["nama"] != "") ? $pembeli["nama"] : "Nama Orang";
        $pembeli["email"] = (isset($pembeli["email"]) AND $pembeli["email"] != "") ? $pembeli["email"] : $globalset->email;
        $pembeli["nohp"] = (isset($pembeli["nohp"]) AND $pembeli["nohp"] != "") ? $pembeli["nohp"] : $globalset->wasap;
    ...
    // PEMBAYARAN TOPUP
    function createPaymentTopup($trx,$metode,$jumlah,$pembeli,$produk=null){
        $set = $this->set("semua");
        $globalset = $this->globalset("semua");
        $trx = $this->func->getSaldoTarik($trx,"semua");
        $produk = is_array($produk) ? $produk : [['sku'=>'BO1','name'=>'Pembelian di '.$globalset->nama,'price'=> $jumlah,'quantity'=>1]];
        $pembeli["nama"] = (isset($pembeli["nama"]) AND $pembeli["nama"] != "") ? $pembeli["nama"] : "Nama Orang";
        $pembeli["email"] = (isset($pembeli["email"]) AND $pembeli["email"] != "") ? $pembeli["email"] : $globalset->email;
        $pembeli["nohp"] = (isset($pembeli["nohp"]) AND $pembeli["nohp"] != "") ? $pembeli["nohp"] : $globalset->wasap;
        ...
        $data = [
            ...
            //'return_url'        => 'https://domainanda.com/redirect',
            'expired_time'      => (time()+(24*60*60)), // 24 jam
            'signature'         => hash_hmac('sha256', $set->merchant_code.$trx->trxid.$jumlah, $set->private_key)
        ];
        ...
        */
        if(!empty($err)){
            ...
            if($response["success"] != 1){
                ...
                $res = array("success"=>true,"msg"=>"Berhasil memproses pembayaran topup");
                $res = (object)$res;
                return $res;
            }
        }
